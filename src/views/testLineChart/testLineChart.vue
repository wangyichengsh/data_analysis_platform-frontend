    ...
    <div class="line" style="width:100%;background:url(http://129.211.26.73:8000/static/js/kcbweb2.png)">
      ...
      <div style="margin-top: 35px;">
        <div class="card" style="width:923px;height:475.2px;float:left;" >
          <priceOverview :simple="false" ></priceOverview>
        </div>
        <div style="float:left;" class="card" >
          <div  id="mychart" style="height:272px;width:445px;" ></div>
        </div>
        <div style="display:none;float:left;" class="card" >
          <div  id="mychart_index" style="height:272px;width:445%;" ></div>
        </div>
        <div style="display:none;float:right;margin-right:0.25%;margin-top: 0.25%;width:45%;" >
          <contri_table></contri_table>
        </div>
        <div class="card" style="float:left;margin-left: 1%;margin-top: 2%;width:53.5%;height:483px;" >
          <heatmap></heatmap>
        </div>
        <div style="display:none;float:right;margin-right:47px;margin-top: 0.25%;width:42%;" >
          <imp_table></imp_table>
        </div>
  ...
  function showXaxis(index, value) {
    ...
    export default {
      ...
      methods:{
        ...
        getTitlePerColorText(data,standard){
          if(Number(data)>Number(standard)){
            return "{up|"+String(data)+"%}"
          }
          else if(Number(data)==Number(standard)){
        ...
        getLatestOpt(data, tit){
          ...
          let title = {
            text:" "+String(tit)+"   现价:"+this.getTitleColorText(data['lineData'][data['lineData'].length-1], data["lastCloseData"])+" ( "+this.getTitleColorText((data['lineData'][data['lineData'].length-1]-data["lastCloseData"]).toFixed(2),0)+" , "+this.getTitlePerColorText(data["rateChange"],0)+") 金额:"+String((data["curr_trade_vol"]/100000000).toFixed(2))+'亿' ,
            textStyle:{
              color:"white",
              fontSize: 14,
              fontWeight: 12,
        ...
        renewChartData(){
          queryKcbWeb2Chart1({'code':'000688'}).then((response) => {
            let newchartData = response.data.results;
            let opt = this.getLatestOpt(newchartData,'科创50');
            this.echartIns.setOption(opt);
      ...
      mounted (){
        queryKcbWeb2Chart1({'code':'000688'}).then((response) => {
          ...
          this.echartIns =  this.drawLineChart('mychart', this.chartData,'科创指数');
        }).catch(function (error) { console.log(error);});
        queryKcbWeb2Chart1({'code':'000001'}).then((response) => {
        ...
        let kcb_ws1 = new WebSocket(channel_host+"/ws/web/KcbLine000688/");
        kcb_ws1.onopen = function () {
          console.log('数据刷新已开启');
        };
        kcb_ws1.onmessage = (evt) => {
          var received_msg = evt.data;
          var msg = JSON.parse(received_msg)['message'];
          this.chartData = msg;
          this.echartIns =  this.drawLineChart('mychart', this.chartData,'科创指数');
        };
        kcb_ws1.onclose = function () {
          // 关闭 websocket
          console.log("数据刷新已关闭");
        };
