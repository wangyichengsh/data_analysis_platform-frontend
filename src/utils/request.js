...
service.interceptors.request.use(
  config => {
    ...
    const ngsp_token = getLocalStore('ngsp_token')
    const if_ngsp = config.url.indexOf('ngsp') > 0
    // 判断使用哪个token
    if (token && !if_ngsp) {
  ...
  */
  ...
  response => {
    ...
    if (status !== 200) {
      ...
      // 50008: Illegal token; 50012: Other clients logged in; 50014: Token expired; 状态代码需待与drf比对更新
